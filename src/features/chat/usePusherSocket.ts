...
import pusherService, { PusherConnectionState } from "../../lib/pusher";

interface UsePusherSocketProps {
  threadId: number;
...
const usePusherSocket = ({
  ...
  useEffect(() => {
    const unsubscribe = pusherService.subscribeToConnectionState((state) => {
  ...
  // Set up Pusher channel when threadId is available
  useEffect(() => {
    if (!threadId || !chatToken) {
      return;
    }

    const channelName = `private-threads.${threadId}`;

    // Unsubscribe from previous channel if exists
    if (channelRef.current) {
      pusherService.unsubscribeFromChannel(channelRef.current);
    }

    // Ensure Pusher is initialized before subscribing
    pusherService.initialize();

    // Subscribe to new channel
    const channel = pusherService.subscribeToPrivateChannel(channelName);
    channelRef.current = channelName;

    if (channel) {
      // Listen for new messages
      channel.bind("MessageSent", (payload: PointerThreadConversationProps) => {
      ...
      channel.bind("ThreadUpdated", (payload: ThreadResponseData) => {
        if (onThreadUpdated) {
  ...
  }, [threadId, chatToken, onThreadUpdated]);
...
export default usePusherSocket;
